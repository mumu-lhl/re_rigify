...
_ZH_HANS_DEFAULT = {
    ...
    "Arrange Collection UI": "编排集合 UI",
    "Auto Arrange Collection UI": "自动编排集合 UI",
    "Arrange bone collections into standard categorized UI rows with empty spacing and generation visibility": (
        "按分类自动编排骨骼集合界面行，保留空行并设置生成可见性"
    ),
    "Arranged UI for {count} collection(s)": "已完成 {count} 个集合的 UI 编排",
    ...
    "Body Part": "身体部位",
    "Head": "头部",
    "Head & neck chain": "头部与颈部链",
    "Torso": "躯干",
    "Spine & chest chain": "脊柱与胸腔链",
    "Arm / Hand": "手部",
    "Shoulder (super_copy) + arm chain (limbs.arm)": "肩膀 (super_copy) + 手臂链 (limbs.arm)",
    "Leg": "腿部",
    ...
    "Align rolls to Global +Z/-Y with -X primary axis for natural fist curling on scale down": (
        "骨骼 Roll 对齐全局 +Z/-Y 并使用 -X 主旋转轴，控制器缩小自然握拳"
    ),
    "Keep Original (AUTO)": "保持原始骨骼轴向 (AUTO)",
    "Keep original bone rolls and automatic rotation axis": "保留骨骼原始 Roll 与 Rigify 自动旋转轴",
    "Enable Finger IK": "启用手指 IK",
    "Generate fingertip IK controls with FK/IK sliders and snapping": "为指尖生成球形 IK 控制器，支持 FK/IK 滑块与吸附",
    "Thumb Roll Alignment": "大拇指 Roll 对齐",
    "Global -Y (MMR Default)": "全局 -Y (MMR 默认对掌)",
    "Align thumb roll to Global -Y for natural inward opposition curl": "大拇指 Roll 对齐全局 -Y，实现自然向掌心内收对掌握拳",
    "Global +Z": "全局 +Z",
    "Align thumb roll to Global +Z (same as other fingers)": "大拇指 Roll 对齐全局 +Z（与其他四指一致）",
    "Mirror to Opposite Side": "对称配置到对侧",
    "Automatically configure opposite side if symmetric (.L/.R)": "若为对称骨骼（.L/.R），自动为对侧生成配置",
    "Selected Bones: {count}": "已选骨骼：{count}",
    "Select one or more bones in Pose or Edit Mode": "请在姿态或编辑模式下选中一根或多根骨骼",
    "No collections to arrange": "没有可编排的集合",
    "Configured {part} for {count} bone(s)": "已为 {count} 根骨骼创建 {part} 配置",
