...
metarig: `AUTO` maps to Rigify's lowercase `automatic`, while an explicit UI
choice maps directly to its signed axis value. This guarantees that a name
change cannot change the generated result.
